...
namespace PgcDemuxCS.DVD.IfoTypes.Common
{
    ...
    /// </summary>
    internal class pgc_t
    {
        internal const uint Size = 236;

        internal ushort zero_1;
        internal byte nr_of_programs;
        internal byte nr_of_cells;
        internal dvd_time_t playback_time;
        internal user_ops_t prohibited_ops;
        internal ushort[] audio_control = new ushort[8]; // TODO: New type? 
        internal uint[] subp_control = new uint[32]; // TODO: New type? 
        internal ushort next_pgc_nr;
        internal ushort prev_pgc_nr;
        internal ushort goup_pgc_nr;
        internal byte pg_playback_mode;
        internal byte still_time;
        internal uint[] palette = new uint[16]; // TODO: New type struct {zero_1, Y, Cr, Cb} ?
        internal ushort command_tbl_offset;
        internal ushort program_map_offset;
        internal ushort cell_playback_offset;
        internal ushort cell_position_offset;
        internal pgc_command_tbl_t? command_tbl = null;

        /// <summary>
        /// PGC Program map
        /// </summary>
        internal byte[] program_map = Array.Empty<byte>();
        internal cell_playback_t[] cell_playback = Array.Empty<cell_playback_t>();
        internal cell_position_t[] cell_position = Array.Empty<cell_position_t>();
